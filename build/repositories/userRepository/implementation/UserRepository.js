...
class UserRepository {
  async save(user) {
    ...
    const {
      rows
    } = await _pool.pool.query(query, [user.Id, user.Username, user.Password, user.Role, user.JID]);
    return {
      id: rows[0].id_user
    };
  }
  ...
  async getUniqueUser({
    ...
    const {
      rows
    } = await _pool.pool.query(query, [id, username]);
    return rows[0];
  }
  ...
  async update({
    ...
    const databaseResponse = await _pool.pool.query(query, [fields.username, fields.password, fields.role, fields.jid, id]);
